...
class TrackerThread(threading.Thread):
    ...
    def run(self):
        self.result = tracking(self.yolo, self.initImg, self.img, self.loc, self.scores, self.lab, self.frame,self.isDraw)

    def get_result(self):
        return self.result


def detect_camera3(yolo,videoPath=None, loop=None, output_path="", trackingAlt=True,isDebug=False,drawTrack=False):
    """
    ...
    video_FourCC    =  cv2.VideoWriter_fourcc('m', 'p', '4', 'v')##int(cap.get(cv2.CAP_PROP_FOURCC))#c vp09 avc1 hvc1 #
    video_fps       = cap.get(cv2.CAP_PROP_FPS)
    video_size      = (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),
                        int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    print('the video video_FourCC = {},video_fps= {},  video_size = {}  output = {}'.format(video_FourCC,video_fps,video_size,output_path))
    ...
    print('captime time is {}  {}%'.format(captime,(captime/(end1_time - start1_time))))
    print('detecttime time is {}  {}%'.format(detecttime, (detecttime / (end1_time - start1_time))))
    print('drawtime time is {}  {}%'.format(drawtime, (drawtime / (end1_time - start1_time))))
    print('gputime time is {}  {}%'.format(yolo.gputime, (yolo.gputime / (end1_time - start1_time))))
    print('cputime time is {}  {}%'.format(yolo.cputime, (yolo.cputime / (end1_time - start1_time))))
...
if __name__=="__main__":
    detect_camera(videoPath="./video/12.avi", loop=10) #videoPath="./video/12.avi"